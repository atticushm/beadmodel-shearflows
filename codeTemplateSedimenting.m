...
b0 = L/(Nb-1)/L;                                  % equilibrium distance between beads.

youngMod = kape*b0/pi/a^2;                      % Young's modulus.
...
xb(1,:,1) = linspace(0,L,Nb)/L;               % equally space nodes in x-plane.
% xb(2,:,1) = intConfig(xb(1,:));
...
for n = 1:Nt
    ...
    %% bending forces.            
    Fb = get_bending_forces(X);
    F = F + (Nb-1).*Fb;

    %% spring forces.
    Fs = get_spring_forces(X, b0);
    F  = F + calS.*Fs;
    ...
    for p = 1:Nb      
        clear stokeslets
        stokeslets = get_stokeslets(X, xb(:,p),epsilon);
        G = reshape(F',[1,3*Nb])';
        U(:,p) = stokeslets*G;
    end 
    ...
    if mod(n,viewInt)== 0
        clf
        hold on
        figBM = draw_bm_buckling(X,Xc);
        xlabel('x')
        ylabel('y')
        title(sprintf('beta=%g; t=%g',beta,t(n)))
        pause(0.01)
        hold off
    end
...
xlabel('x')
ylabel('y')
title(sprintf('Final config. @ t = %g; S = %g, G = %g, w = %g',t(n), calS, calG, abs(weight)))
hold off
...
axis([-0.1 1.1 xc(2)-0.08 xc(2)+0.08 0 1]) 
view(2)
% axis equal
xlabel('x')
ylabel('y')
